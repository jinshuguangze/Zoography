...
public class AutoHandle {
	...
	protected HashMap<Integer,String> autoItemCountConformity(List<?> mainList, List<String> valueList, List<Integer> keyList)
		...
		if (mainCount > valueCount) {
				...
				ConfigHandle.setConfigData("Option.cfg", "ListViewItems", valueList.toString().substring(1, valueList.toString().length()-1).replace(" ", ""));
		} 
		...
		if (mainCount > keyCount) {
			...
			ConfigHandle.setConfigData("Option.cfg", "ListViewEventSequence", keyList.toString().substring(1, keyList.toString().length()-1).replace(" ", ""));
		} 
